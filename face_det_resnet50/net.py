...
class RetinaFace(nn.Module):
    def __init__(self):
        ...
        in_channel = 256
        out_channel = 256
        self.body = torchvision.models._utils.IntermediateLayerGetter(backbone, return_layers)
        in_channels_list = [
            in_channel * 2,
            in_channel * 4,
            in_channel * 8
    ...
    def forward(self, images):
        ...
        landmark_reg = torch.cat([self.LandmarkHead[i](feature) for i, feature in enumerate(features)], dim=1)

        output = (bbox_regressions, F.softmax(classifications, dim=-1), landmark_reg)
        return output
